...
function FloatingQuotes({ quotes }) {
  ...
  // Keep quotesRef always current — when language changes, new quotes are used immediately
  useEffect(() => {
    quotesRef.current = quotes
    // Clear any visible quotes from the old language instantly
    setVisible([])
  }, [quotes])
  ...
  useEffect(() => {
    ...
    let i = 0
    const QUOTE_DIRS = ['left','right','left','right','left','right']
    const QUOTE_COLORS = ['#14b8a6','#0d9488','#14b8a6','#0d9488','#14b8a6','#0d9488']
    const spawn = () => {
      if (!activeRef.current) return
      const currentQuotes = quotesRef.current
      const idx = i % currentQuotes.length
      const id = counterRef.current++
      const top = 12 + Math.random() * 76
      const dir = QUOTE_DIRS[idx]
      const color = QUOTE_COLORS[idx]
      setVisible(prev => [...prev, { text: currentQuotes[idx], dir, color, id, top }])
      setTimeout(() => { setVisible(prev => prev.filter(x => x.id !== id)) }, 4200)
      i++
      setTimeout(spawn, 1400 + Math.random() * 900)
    }
  ...
  }, []) // runs once — quotes are always read from quotesRef

  return (
    <>
      {visible.map(q => (
        <div key={q.id} style={{ position:'absolute', top:`${q.top}%`, left:q.dir==='left'?0:'auto', right:q.dir==='right'?0:'auto', zIndex:2, pointerEvents:'none', animation:q.dir==='left'?'slideInLeft 0.55s cubic-bezier(.22,1,.36,1) forwards, fadeOutSide 0.6s 3.5s ease forwards':'slideInRight 0.55s cubic-bezier(.22,1,.36,1) forwards, fadeOutSide 0.6s 3.5s ease forwards' }}>
          <div style={{ background:'rgba(255,255,255,0.95)', border:`1.5px solid ${q.color}33`, borderLeft:q.dir==='left'?`3px solid ${q.color}`:undefined, borderRight:q.dir==='right'?`3px solid ${q.color}`:undefined, borderRadius:q.dir==='left'?'0 10px 10px 0':'10px 0 0 10px', padding:'10px 16px', fontSize:12.5, fontWeight:600, color:'#0f172a', boxShadow:`0 4px 20px rgba(20,184,166,0.12)`, backdropFilter:'blur(8px)', display:'flex', alignItems:'center', gap:8, maxWidth:260, whiteSpace:'nowrap' }}>
...
export default function Hero({ t }) {
  ...
  return (
    <section style={s.hero}>
      ...
      <div style={s.grid}/>
      <FloatingQuotes quotes={h.quotes} />
      <div style={s.inner}>
        <div style={{...s.tag,...(mounted?s.tagIn:s.tagOut)}}>
          <div style={s.pulseWrap}><div style={s.pulseRing}/><div style={s.pulseDot}/></div>
          {h.badge}
        </div>
        <h1 style={s.h1}>
          <span style={{display:'block',...(mounted?s.slideL:s.slideOut)}}>{h.line1}</span>
          <span style={{display:'block',color:'#14b8a6',...(mounted?s.slideR:s.slideOut)}}>{h.line2}</span>
        </h1>
        ...
        <div style={{...s.kpiBar,...(mounted?s.fadeIn5:s.fadeOut)}}>
          {kpis.map(([n, l], i) => (
            <div key={l} style={{...s.kpi,...(i===3?{borderRight:'none'}:{}),...(kpiHover===i?s.kpiH:{})}} onMouseEnter={()=>setKpiHover(i)} onMouseLeave={()=>setKpiHover(null)}>
              <span style={s.kn}>
                {String(n).startsWith('<') && <><span style={s.kdim}>&lt;</span><em style={s.em}>{String(n).slice(1)}</em></>}
                {!String(n).startsWith('<') && !String(n).endsWith('%') && <span style={kpiHover===i?s.em:{color:'#14b8a6'}}>{n}</span>}
                {String(n).endsWith('%') && <><em style={s.em}>{String(n).replace('%','')}</em><span style={s.kdim}>%</span></>}
              </span>
        ...
        <div style={{...s.ticker,...(mounted?s.fadeIn5:s.fadeOut)}}>
          ...
          <span style={s.tickerTxt}><TypewriterText text={h.ticker} delay={2200}/></span>
        </div>
        <div style={{...s.scroll,...(mounted?s.fadeIn5:s.fadeOut)}}>
          <div style={s.scrollDot}/>
          <span style={s.scrollTxt}>{h.scroll}</span>
        </div>
      </div>
      <style>{`
...
const tr = (delay='0s') => ({ transition:`opacity .7s ${delay} ease, transform .7s ${delay} cubic-bezier(.22,1,.36,1)` })
const s = {
  ...
  tag:{display:'inline-flex',alignItems:'center',gap:10,background:'rgba(20,184,166,0.07)',border:'1px solid rgba(20,184,166,0.25)',borderRadius:40,padding:'6px 18px 6px 8px',fontSize:12.5,color:'#14b8a6',fontWeight:600,marginBottom:34,letterSpacing:.4,backdropFilter:'blur(8px)',...tr('0s')},
  tagOut:{opacity:0,transform:'translateY(-16px)'},tagIn:{opacity:1,transform:'translateY(0)'},
  ...
  slideOut:{opacity:0,transform:'translateX(-60px)',...tr('0s')},slideL:{opacity:1,transform:'translateX(0)',...tr('.12s')},slideR:{opacity:1,transform:'translateX(0)',...tr('.28s')},
  sub:{fontSize:'clamp(15px,1.8vw,19px)',color:'#1e4d47',maxWidth:580,margin:'20px auto 44px',fontWeight:400,lineHeight:1.75,...tr('0s')},
  fadeOut:{opacity:0,transform:'translateY(20px)'},fadeIn3:{opacity:1,transform:'translateY(0)',...tr('.42s')},fadeIn4:{opacity:1,transform:'translateY(0)',...tr('.56s')},fadeIn5:{opacity:1,transform:'translateY(0)',...tr('.72s')},
...
btnStore:{background:'#14b8a6',color:'#fff',padding:'14px 30px',borderRadius:8,fontSize:15,fontWeight:700,textDecoration:'none',border:'none',display:'inline-flex',alignItems:'center',gap:8,transition:'all .25s ease',boxShadow:'0 4px 20px rgba(20,184,166,.2)'},  kpiBar:{display:'flex',background:'rgba(255,255,255,0.92)',border:'1px solid rgba(20,184,166,0.2)',borderRadius:14,overflow:'hidden',boxShadow:'0 4px 24px rgba(20,184,166,.08)',backdropFilter:'blur(12px)'},
  ...
  em:{color:'#14b8a6',fontStyle:'normal'},kdim:{color:'#94a3b8',fontStyle:'normal'},
